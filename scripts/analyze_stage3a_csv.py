#!/usr/bin/env python3
"""Offline Stage 3A CSV inventory and quick-look plots.
...
DEFAULT_OUTPUT_DIR = Path("outputs/stage3a_analysis")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze Stage 3A offline CSV logs without ROS2.",
    )
    parser.add_argument(
        "csv_paths",
        nargs="*",
    ...
    parser.add_argument(
        ...
        help=f"Expected sample count for a complete Stage 3A run. Default: {EXPECTED_SAMPLES}",
    )
    parser.add_argument(
        "--expected-rounds",
        type=int,
...
def find_csv_files(args: argparse.Namespace) -> list[Path]:
    ...
    if not args.input_dir.exists():
        return []
    return sorted(args.input_dir.rglob("*.csv"))
...
def infer_stage3a_mode(path: Path) -> str:
...
def analyze_csv(
    ...
    try:
        ...
        return {
            ...
            "_columns_tuple": tuple(),
        }

    print(f"mode: {infer_stage3a_mode(path)}")
    print(f"run status: {infer_run_status(path)}")
    print(f"rows: {len(df)}")
    ...
    if len(df) == expected_samples:
        ...
        notes.append("comparability caveat: not a complete 6-round validation run")
    ...
    return {
        ...
        "columns_match": False,
        "expected_samples": expected_samples,
        "sample_completion_ratio": len(df) / expected_samples if expected_samples else math.nan,
        ...
        "notes": "; ".join(notes),
        "_columns_tuple": tuple(df.columns),
    }
...
def print_final_summary(summary: pd.DataFrame) -> None:
    ...
    comparable_columns = bool(summary["columns_match"].all())
...
def main() -> int:
    ...
    rows = [
        analyze_csv(path, args.input_dir, args.output_dir, args.expected_samples, args.expected_rounds)
        for path in csv_files
    ]
    summary = pd.DataFrame(rows)
    if not summary.empty and "_columns_tuple" in summary:
        reference_columns = summary["_columns_tuple"].iloc[0]
        summary["columns_match"] = summary["_columns_tuple"].apply(lambda columns: columns == reference_columns)
        summary = summary.drop(columns=["_columns_tuple"])
    summary_path = args.output_dir / "stage3a_summary.csv"
