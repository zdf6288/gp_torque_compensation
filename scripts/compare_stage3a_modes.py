#!/usr/bin/env python3
"""Compare Stage 3A offline CSV modes.

This script has no ROS2 dependency. It loads saved Stage 3A CSV logs, detects
...
DEFAULT_INPUT_DIR = Path("data/stage3a/csv")
DEFAULT_OUTPUT_DIR = Path("outputs/stage3a_comparison")
EXPECTED_SAMPLES = 3749
EPS = 1e-9

MODE_SETTINGS = {
    "comp_off": (0.0, math.nan),
    "gp_on_scale03": (0.3, 1.0),
    "gp_on_scale05": (0.5, 1.0),
    "gp_on_scale07": (0.7, 1.0),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare Stage 3A CSV modes offline without ROS2.",
    )
    parser.add_argument(
        "csv_paths",
        nargs="*",
        type=Path,
        help="Specific CSV files to compare. If omitted, all CSVs under --input-dir are used recursively.",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fallback scale for unknown modes in clip proxy calculation. Stage 3A known modes infer scale from filename. Default: 0.1",
    )
    parser.add_argument(
        "--gp-clip-nm",
        type=float,
        default=0.5,
        help="Fallback clip threshold in Nm for unknown modes. Stage 3A known modes infer clip from filename. Default: 0.5",
    )
...
def find_csv_files(csv_paths: list[Path], input_dir: Path, include: list[str], exclude: list[str]) -> list[Path]:
    if csv_paths:
        paths = sorted(path for path in csv_paths if path.suffix.lower() == ".csv")
    else:
...
def relative_to_input(path: Path, input_dir: Path) -> str:
    try:
...
def infer_mode_name(path: Path) -> str:
    ...
    parent = normalized_name(path.parent.name)
    text = f"{parent}_{stem}"
    if "comp_off" in text:
        return "comp_off"
    if "scale05" in text:
        return "gp_on_scale05"
    if "scale07" in text:
        return "gp_on_scale07"
    if "scale03" in text or "conservative" in text:
        return "gp_on_scale03"
    return "unknown"


def infer_run_status(path: Path) -> str:
    stem = normalized_name(path.stem)
    for status in ("fullrun", "partial", "nearfull", "short", "attempt"):
        if status in stem:
            return status
    return "unknown"


def mode_scale_clip(mode_name: str, fallback_scale: float, fallback_clip_nm: float) -> tuple[float, float]:
    return MODE_SETTINGS.get(mode_name, (fallback_scale, fallback_clip_nm))


def comparability_caveat(run_status: str, rows: int, expected_samples: int) -> str:
    caveats = []
    if run_status != "fullrun":
        caveats.append(f"{run_status} run; not a complete 6-round validation")
    if rows != expected_samples:
        caveats.append(f"rows {rows} differ from expected {expected_samples}")
    return "; ".join(caveats)
...
def load_csv(path: Path, input_dir: Path, fallback_scale: float, fallback_clip_nm: float) -> dict[str, object] | None:
    try:
    ...
    run_status = infer_run_status(path)
    scale, clip_nm = mode_scale_clip(mode_name, fallback_scale, fallback_clip_nm)
    print(f"Loaded {path.name}: mode={mode_name}, status={run_status}, rows={rows}, columns={len(columns)}")
    return {
        "path": path,
        "relative_path": relative_to_input(path, input_dir),
        "columns": columns,
        "data": arrays,
        "rows": rows,
        "mode_name": mode_name,
        "run_status": run_status,
        "gp_compensation_scale": scale,
        "gp_compensation_clip_nm": clip_nm,
    }


def load_datasets(csv_files: list[Path], input_dir: Path, fallback_scale: float, fallback_clip_nm: float) -> list[dict[str, object]]:
    datasets = []
    for path in csv_files:
        dataset = load_csv(path, input_dir, fallback_scale, fallback_clip_nm)
...
def make_timing_summary(dataset: dict[str, object]) -> dict[str, object]:
    ...
    row = {
        ...
        "sample_completion_ratio": dataset["rows"] / EXPECTED_SAMPLES if EXPECTED_SAMPLES else math.nan,
        "gp_compensation_scale": dataset["gp_compensation_scale"],
        "gp_compensation_clip_nm": dataset["gp_compensation_clip_nm"],
        "comparability_caveat": comparability_caveat(dataset["run_status"], dataset["rows"], EXPECTED_SAMPLES),
...
def make_cartesian_tracking_summary(
    ...
    for dataset in datasets:
        ...
        for axis, actual_col, desired_col in zip(("x", "y", "z"), actual, desired):
            ...
            row[f"mean_error_{axis}"] = float(np.mean(finite)) if len(finite) else math.nan
        ...
        if len(finite_rows):
            ...
            row["mean_norm"] = float(np.mean(norms))
            row["rmse_norm"] = rms(norms)
            row["max_norm"] = float(np.max(norms))
            row["p95_norm"] = float(np.percentile(norms, 95))
        else:
            row["mean_norm"] = math.nan
            row["rmse_norm"] = math.nan
            row["max_norm"] = math.nan
...
def make_tau_like_row(dataset: dict[str, object], joint: int, column: str, metric_group: str) -> dict[str, object]:
...
def make_prediction_row(dataset: dict[str, object], joint: int, column: str, prediction_group: str) -> dict[str, object]:
    values = finite_values(dataset["data"][column])
    mean = float(np.mean(values)) if len(values) else math.nan
    std = float(np.std(values)) if len(values) else math.nan
...
def make_joint_metric_rows(
    ...
    for dataset in datasets:
        ...
        for joint, column in joint_columns:
            if metrics == "tau_like":
                rows.append(make_tau_like_row(dataset, joint, column, metric_group or group_label))
            elif metrics == "prediction":
                rows.append(make_prediction_row(dataset, joint, column, metric_group or group_label))
    return rows, detected
...
def make_clip_proxy_rows(
    ...
    for dataset in datasets:
        path = dataset["path"]
        gp_scale = float(dataset["gp_compensation_scale"])
        gp_clip_nm = float(dataset["gp_compensation_clip_nm"])
        threshold = gp_clip_nm * 0.98 if np.isfinite(gp_clip_nm) else math.nan
        joint_columns = y_hat_detected.get(path.name, [])
        for joint, column in joint_columns:
            values = finite_values(dataset["data"][column])
            scaled_abs = np.abs(gp_scale * values)
            clip_proxy = scaled_abs >= threshold if np.isfinite(threshold) else np.zeros_like(scaled_abs, dtype=bool)
            rows.append({
                "mode_name": dataset["mode_name"],
                "run_status": dataset["run_status"],
                "csv_file": path.name,
                ...
                "gp_compensation_clip_nm": gp_clip_nm,
...
def make_tau_residual_comparison(residual_metrics: list[dict[str, object]]) -> list[dict[str, object]]:
    by_mode_joint = {
        ...
        if row.get("run_status") == "fullrun" and row.get("metric_group") == "tau_residual"
    }
    modes = {mode for mode, _joint in by_mode_joint}
    if not {"comp_off", "gp_on_scale03"}.issubset(modes):
        print("WARNING: comp_off fullrun and gp_on_scale03 fullrun were not both detected; skipping residual comparison.")
        return []

    rows = []
    for joint in range(1, 8):
        comp_off_rms = by_mode_joint.get(("comp_off", joint), math.nan)
        scale03_rms = by_mode_joint.get(("gp_on_scale03", joint), math.nan)
        if not np.isfinite(comp_off_rms) or not np.isfinite(scale03_rms):
            continue
        rows.append({
            "comparison": "gp_on_scale03_fullrun_vs_comp_off_fullrun",
            "joint": joint,
            "rms_comp_off": comp_off_rms,
            "rms_gp_on_scale03": scale03_rms,
            "rms_change_percent": 100.0 * (scale03_rms - comp_off_rms) / comp_off_rms if comp_off_rms else math.nan,
        })
    return rows
...
def is_representative_dataset(dataset: dict[str, object]) -> bool:
    return (
        (dataset["mode_name"] == "comp_off" and dataset["run_status"] == "fullrun")
        or (dataset["mode_name"] == "gp_on_scale03" and dataset["run_status"] == "fullrun")
        or (dataset["mode_name"] == "gp_on_scale05" and dataset["run_status"] == "partial")
        or (dataset["mode_name"] == "gp_on_scale07" and dataset["run_status"] == "nearfull")
    )


def representative_datasets(datasets: list[dict[str, object]]) -> list[dict[str, object]]:
    selected = [dataset for dataset in datasets if is_representative_dataset(dataset)]
    return selected if selected else datasets


def row_plot_label(row: dict[str, object]) -> str:
    mode = str(row["mode_name"])
    status = str(row.get("run_status", ""))
    if status and status != "fullrun":
...
def save_cartesian_error_overlay(
    ...
    for dataset in datasets:
        ...
        label = row_plot_label(dataset)
        ax.plot(x, norms, label=label, linewidth=0.9)
...
def save_actual_vs_desired_plots(
    ...
    for dataset in datasets:
        ...
        label = row_plot_label(dataset)
        fig.suptitle(f"Cartesian actual vs desired: {label}")
        fig.tight_layout()
        output_path = output_dir / f"stage3a_cartesian_actual_vs_desired_{path.stem}.png"
        output_path.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(output_path, dpi=140)
        plt.close(fig)
        count += 1
...
def save_joint_bar_plot(
    ...
    modes = sorted({row_plot_label(row) for row in rows})
    joints = sorted({int(row["joint"]) for row in rows})
    if not modes or not joints:
        return False

    values = {
        (row_plot_label(row), int(row["joint"])): float(row[value_column])
        for row in rows
...
def main() -> int:
    ...
    csv_files = find_csv_files(args.csv_paths, args.input_dir, args.include, args.exclude)
    if not csv_files:
    ...
    datasets = load_datasets(csv_files, args.input_dir, args.gp_scale, args.gp_clip_nm)
    if not datasets:
        print("WARNING: no CSV files could be loaded.")
        return 1

    timing_summary = [make_timing_summary(dataset) for dataset in datasets]
    write_rows(args.output_dir / "stage3a_timing_summary.csv", timing_summary, TIMING_FIELDS)

    cartesian_summary, cartesian_detected = make_cartesian_tracking_summary(datasets)
    write_rows(args.output_dir / "stage3a_cartesian_tracking_summary.csv", cartesian_summary, CARTESIAN_FIELDS)
    ...
    residual_metrics, residual_detected = make_joint_metric_rows(
        ...
        metrics="tau_like",
        metric_group="tau_residual",
    )
    raw_residual_metrics, raw_residual_detected = make_joint_metric_rows(
        datasets,
        prefixes=("tau_residual_raw", "raw_tau_residual"),
        group_label="raw tau residual",
        metrics="tau_like",
        metric_group="tau_residual_raw",
    )
    residual_metrics_all = residual_metrics + raw_residual_metrics
    write_rows(args.output_dir / "stage3a_tau_residual_metrics.csv", residual_metrics_all, TAU_LIKE_FIELDS)

    residual_comparison = make_tau_residual_comparison(residual_metrics_all)
    write_rows(args.output_dir / "stage3a_tau_residual_comparison.csv", residual_comparison, RESIDUAL_COMPARISON_FIELDS)

    y_hat_stats, y_hat_detected = make_joint_metric_rows(
        datasets,
        prefixes=("y_hat", "gp_prediction"),
        group_label="GP prediction",
        metrics="prediction",
        metric_group="y_hat",
    )
    y_hat_local_stats, y_hat_local_detected = make_joint_metric_rows(
        datasets,
        prefixes=("y_hat_local",),
        group_label="local GP prediction",
        metrics="prediction",
        metric_group="y_hat_local",
    )
    y_hat_cloud_stats, y_hat_cloud_detected = make_joint_metric_rows(
        datasets,
        prefixes=("y_hat_cloud",),
        group_label="cloud GP prediction",
        metrics="prediction",
        metric_group="y_hat_cloud",
    )
    y_hat_mem_stats, y_hat_mem_detected = make_joint_metric_rows(
        datasets,
        prefixes=("y_hat_mem",),
        group_label="memory GP prediction",
        metrics="prediction",
        metric_group="y_hat_mem",
    )
    gp_prediction_stats = y_hat_stats + y_hat_local_stats + y_hat_cloud_stats + y_hat_mem_stats
    write_rows(args.output_dir / "stage3a_gp_prediction_stats.csv", gp_prediction_stats, GP_PREDICTION_FIELDS)

    clip_proxy = make_clip_proxy_rows(datasets, y_hat_detected)
    if not clip_proxy:
        print("WARNING: no y_hat-like columns found; clip proxy summary is empty.")
    write_rows(args.output_dir / "stage3a_clip_proxy.csv", clip_proxy, CLIP_PROXY_FIELDS)
    ...
    write_rows(args.output_dir / "stage3a_tau_metrics.csv", tau_metrics, TAU_LIKE_FIELDS)

    plot_dir = args.output_dir / "plots"
    plots_written = []
    plot_datasets = representative_datasets(datasets)
    plot_files = {dataset["path"].name for dataset in plot_datasets}
    residual_plot_rows = [
        row for row in residual_metrics
        if row["csv_file"] in plot_files and row.get("metric_group") == "tau_residual"
    ]
    y_hat_plot_rows = [row for row in y_hat_stats if row["csv_file"] in plot_files]
    clip_plot_rows = [row for row in clip_proxy if row["csv_file"] in plot_files]
    tau_plot_rows = [row for row in tau_metrics if row["csv_file"] in plot_files]

    if save_cartesian_error_overlay(
        plot_datasets,
        cartesian_detected,
        plot_dir / "stage3a_cartesian_error_norm_overlay.png",
    ):
        plots_written.append("stage3a_cartesian_error_norm_overlay.png")
    actual_vs_desired_count = save_actual_vs_desired_plots(plot_datasets, cartesian_detected, plot_dir)
    ...
    if save_joint_bar_plot(
        ...
        "RMS",
        plot_dir / "stage3a_tau_residual_rms_by_joint.png",
    ):
        plots_written.append("stage3a_tau_residual_rms_by_joint.png")
    if save_joint_bar_plot(
        y_hat_plot_rows,
        "std",
        "y_hat std by joint",
        "std",
        plot_dir / "stage3a_y_hat_std_by_joint.png",
    ):
        plots_written.append("stage3a_y_hat_std_by_joint.png")
    if save_joint_bar_plot(
        y_hat_plot_rows,
        "range",
        "y_hat range by joint",
        "range",
        plot_dir / "stage3a_y_hat_range_by_joint.png",
    ):
        plots_written.append("stage3a_y_hat_range_by_joint.png")
    if save_joint_bar_plot(
        clip_plot_rows,
        "clip_proxy_ratio",
        "Clip proxy ratio by joint",
        "ratio",
        plot_dir / "stage3a_clip_proxy_ratio_by_joint.png",
    ):
        plots_written.append("stage3a_clip_proxy_ratio_by_joint.png")
    if save_joint_bar_plot(
        tau_plot_rows,
        "rms",
        "Tau RMS by joint",
        "RMS",
        plot_dir / "stage3a_tau_rms_by_joint.png",
    ):
        plots_written.append("stage3a_tau_rms_by_joint.png")

    print("\nMetric groups:")
    print(f"  timing: {len(timing_summary)} rows")
    print(f"  cartesian tracking: {len(cartesian_summary)} rows")
    print(f"  tau residual: {len(residual_metrics_all)} rows")
    ...
    if not residual_detected and not raw_residual_detected:
        print("WARNING: tau residual metric group was unavailable for all CSVs.")
    if not (y_hat_detected or y_hat_local_detected or y_hat_cloud_detected or y_hat_mem_detected):
