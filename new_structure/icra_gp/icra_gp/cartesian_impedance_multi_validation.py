...
from custom_msgs.msg import StateParameter, EffortCommand, TaskSpaceCommand, DataForGP
...
class CartesianImpedanceMultiValidation(Node):
    ...
    def __init__(self):
        super().__init__('cartesian_impedance_multi_validation')
        ...
        self.declare_parameter('k_gains', [750.0, 750.0, 100.0, 10.0, 10.0, 0.0])    # k_gains in impedance control (task space)
        ...
        self.joint_position_service_called = False  # flag for joint position service called
        ...
        self.time_history = []
        self.x_history = []
        # list for recording data after GP prediction
    ...
    def dataRecordingCallback(self, msg):
        ...
        if msg.data:
            ...
            if not self.gp_predict_finished \
    ...
    def save_data_to_file(self):
        ...
        try:
            filename = 'validation_multi_data_before_gp.csv'
            ...
            filename = 'validation_multi_data_after_gp.csv'
            ...
            self.get_logger().info(f'Successfully saved {len(self.time_history_new)} data points to {filename}')
...
def main(args=None):
    ...
    cartesian_impedance_multi_validation_node = CartesianImpedanceMultiValidation()

    try:
        rclpy.spin(cartesian_impedance_multi_validation_node)
    except KeyboardInterrupt:
        cartesian_impedance_multi_validation_node.get_logger().info('Received keyboard interrupt, saving data...')
    except Exception as e:
        cartesian_impedance_multi_validation_node.get_logger().error(f'Error when running program: {str(e)}')
    finally:
        try:
            # save data to file only if signal handler has not been executed
            if not cartesian_impedance_multi_validation_node._signal_handled:
                cartesian_impedance_multi_validation_node.get_logger().info('Signal handler not executed, saving data to file...')
                cartesian_impedance_multi_validation_node.save_data_to_file()
            else:
                cartesian_impedance_multi_validation_node.get_logger().info('Signal handler executed, data already saved, skipping...')
                
        except Exception as e:
            cartesian_impedance_multi_validation_node.get_logger().error(f'Error when saving data: {str(e)}')
        
        cartesian_impedance_multi_validation_node.destroy_node()
        rclpy.shutdown()
