...
# Global variable to store the node instance for signal handling
cartesian_node_instance = None
...
class CartesianImpedanceMultiData(Node):
    ...
    def __init__(self):
        ...
        self._shutdown_requested = False            # flag to indicate shutdown was requested
...
def main(args=None):
    global cartesian_node_instance
    rclpy.init(args=args)
    cartesian_node_instance = CartesianImpedanceMultiData()

    try:
        rclpy.spin(cartesian_node_instance)
    except KeyboardInterrupt:
        cartesian_node_instance.get_logger().info('Received keyboard interrupt, saving data...')
    except Exception as e:
        cartesian_node_instance.get_logger().error(f'Error when running program: {str(e)}')
    finally:
        try:
            # save data to file only if signal handler has not been executed
            if not cartesian_node_instance._signal_handled:
                cartesian_node_instance.get_logger().info('Signal handler not executed, saving data to file...')
                cartesian_node_instance.save_data_to_file()
            else:
                cartesian_node_instance.get_logger().info('Signal handler executed, data already saved, skipping...')
                
        except Exception as e:
            cartesian_node_instance.get_logger().error(f'Error when saving data: {str(e)}')
        
        cartesian_node_instance.destroy_node()
        rclpy.shutdown()
