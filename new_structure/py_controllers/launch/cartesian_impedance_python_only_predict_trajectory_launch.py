...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            'run_name',
            default_value='MANUAL_F50_GP_PREDICT_ONLY_STARTUP_TRAJ_TEST',
        ...
        LogInfo(
            msg=(
                'PYTHON-ONLY GP PREDICT-ONLY TRAJECTORY: expects external '
                'IMPL bringup and cpp_relayer to be active already. Starts '
                'only cartesian_impedance and trajectory_publisher. GP '
                'prediction is enabled, online update and torque compensation '
                'are disabled, and fixed-start anchoring remains disabled.'
            )
        ),
        controller_node,
        trajectory_node,
